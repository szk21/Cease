    ...
    public class VerticalProgressBar : ProgressBar
    {
        ...
        protected override void WndProc(ref   Message m)
        {
            ...
            if (m.Msg == 0xf || m.Msg == 0x133)
            {
                ...
                System.Drawing.SolidBrush brush = new System.Drawing.SolidBrush(_TextColor);
                string s = string.Format("{0}%", this.Value * 100 / this.Maximum);
                
                System.Drawing.SizeF size = g.MeasureString(s, _TextFont);
                float x = (this.Width - size.Width) / 2;
                float y = (this.Height - size.Height);

                if (this.Value * this.Height / this.Maximum < size.Height)
                {
                    brush = new System.Drawing.SolidBrush(_TextColor);
                }
                else
                {
