...
namespace CeaseUI
{
    ...
    public class ControlImage
    {
        ...
        public static  void SaveControlImage(Control ctrl, string pictureDir, string pictureName)
        {
            try
            {
                Bitmap _BitMap = new Bitmap(ctrl.Width, ctrl.Height);
                ctrl.DrawToBitmap(_BitMap, new Rectangle(0, 0, _BitMap.Width, _BitMap.Height));
                if (!Directory.Exists(pictureDir))
                {
                    Directory.CreateDirectory(pictureDir);
                }
                _BitMap.Save(pictureDir + pictureName);
            }
            catch
            { 
